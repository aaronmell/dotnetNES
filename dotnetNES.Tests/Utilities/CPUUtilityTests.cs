...
namespace dotnetNES.Tests.Utilities
{
    public class CPUUtilityTests
    {
        ...
        public void Loads_Memory_Correctly()
        {
            ...
            Assert.AreEqual(1, cpu.Memory.ReadValue(0x8000));
            Assert.AreEqual(1, cpu.Memory.ReadValue(0xBFFF));

            Assert.AreEqual(2, cpu.Memory.ReadValue(0xC000));
            Assert.AreEqual(2, cpu.Memory.ReadValue(0xFFFF));
        }
    }
}
